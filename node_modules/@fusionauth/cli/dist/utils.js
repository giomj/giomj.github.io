...
dotenv.config();
export const posthogClient = new PostHog('phc_nB6C2uZX2LA6ce6VAaWZxBYPtq1wYH5x8A3n36DaLzQ', { host: 'https://us.i.posthog.com' });
 ...
 */
export const getMessageErrorMessage = (action, messageTemplateId) => {
    let templateIdMessage = 'templates';
 ...
 * @param output
 */
export const getMessageSuccessMessage = (messageTemplateId, output) => {
    let templateIdMessage = 'templates';
    if (messageTemplateId) {
        templateIdMessage = `template ${messageTemplateId}`;
    }
...
export function loadConfig() {
    ...
    try {
        ...
        return { globalConfig: Object.assign(Object.assign({}, defaultConfig), globalConfig) };
    }
    catch (e) {
        return { globalConfig: defaultConfig };
    }
}
export function logEvent(eventName, eventDetails = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!config.globalConfig.telemetryNoWarn) {
            ...
            console.log(boxen(chalk.greenBright.bold('By default, we collect a small amount of anonymous data around CLI usage\n\nYou can disable telemetry by running ')
...
export function createConfig(dir, configObject = { id: randomUUID(), telemetry: true }) {
    ...
    if (!fs.existsSync(configPath)) {
        ...
        if (!config.id || !config.telemetry) {
            ...
            fs.writeFileSync(configPath, JSON.stringify(Object.assign({ id: config.id || randomUUID(), telemetry: config.telemetry === false ? false : true }, config)));
            return fs.existsSync(configPath);
        }
...
function updateGlobalConfig(propertiesToAdd) {
    return __awaiter(this, void 0, void 0, function* () {
        const config = loadConfig();
        const configPath = __dirname + '/.fa/config.json';
        let newConfig;
        if (Array.isArray(propertiesToAdd)) {
            newConfig = Object.assign({}, config.globalConfig);
            ...
            newConfig = Object.assign(Object.assign({}, config.globalConfig), propertiesToAdd);
        }
