...
export class VariableSubstitutor {
     ...
     */
    initialize(variables, kickstartFilePath) {
        this.variables = new Map();
        ...
        for (const [key, value] of Object.entries(VariableSubstitutor.DEFAULT_VARIABLES)) {
     ...
     */
    initializeWithDynamicVariables(variables, kickstartFilePath, apiKey, host) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.variables.has('DEFAULT_TENANT_ID')) {
                try {
                    ...
                    const response = yield client.retrieveApplications();
                    if (!response.wasSuccessful() || !response.response.applications) {
                        throw new Error('Failed to retrieve applications from FusionAuth');
                    }
                    const fusionAuthApp = response.response.applications.find((app) => app.name === 'FusionAuth');
                    if (!fusionAuthApp) {
                        throw new Error('Application named "FusionAuth" not found. Please ensure the application exists in your FusionAuth instance.');
                    }
                    if (!fusionAuthApp.tenantId) {
                        throw new Error('The "FusionAuth" application does not have an associated tenant ID');
                    }
                    // Store the tenant ID
                    ...
                    throw new Error(`Failed to fetch DEFAULT_TENANT_ID from FusionAuth: ${message}`);
                }
            }
        });
    }
     ...
     */
    substituteInObject(obj, resolved) {
        ...
        try {
            ...
            const errorMessages = unresolvedVariables.map((v) => `Unresolved variable: #{${v}}`);
            return {
     ...
     */
    substituteInString(str, resolved) {
        const unresolvedVariables = [];
        const errors = [];
        try {
     ...
     */
    substituteRequest(request, resolved) {
        const errors = [];
        // Substitute URL
        ...
        if (tenantId) {
            ...
            if (!tenantResult.success) {
                errors.push(`TenantId substitution failed: ${tenantResult.errors.join(', ')}`);
            }
            else {
     ...
     */
    deepSubstitute(value, resolved, unresolvedVariables) {
        if (typeof value === 'string') {
            return this.substituteString(value, resolved, unresolvedVariables);
        }
        if (Array.isArray(value)) {
            return value.map((item) => this.deepSubstitute(item, resolved, unresolvedVariables));
        }
     ...
     */
    substituteString(str, resolved, unresolvedVariables) {
        let result = str;
        const fileInclusions = new Map();
     ...
     */
    resolveVariable(varName, resolved) {
     ...
     */
    includeFile(filePath, jsonEscape) {
     ...
     */
    findUnresolvedVariables(config, resolved) {
     ...
     */
    findUnresolvedInString(str, resolved, unresolved) {
     ...
     */
    findUnresolvedInObject(obj, resolved, unresolved) {
     ...
     */
    getPromptedVariables(variables) {
     ...
     * @returns Map of variable name to prompt text
     */
    getHiddenPromptedVariables(variables) {
        const prompted = new Map();
        for (const [key, value] of Object.entries(variables)) {
            if (this.isHiddenPromptVariable(value)) {
