...
export class LineTracker {
     ...
     */
    static getArrayLineNumbers(filePath, arrayPath) {
        const content = fs.readFileSync(filePath, 'utf-8');
        const lines = content.split('\n');
        const lineMap = {};
        let state = 'seekingProperty';
        ...
        const processChar = (char, lineNumber) => {
            if (escapeNext) {
                escapeNext = false;
                return true;
            }
            ...
            if (char === '[') {
                ...
                bracketDepth--;
                if (bracketDepth === 0)
                    return false; // closing ] of the target array
            }
            else if (char === '{') {
                if (objectDepth === 0)
                ...
                if (propIdx !== -1) {
                    ...
                    if (bracketIdx !== -1) {
                        ...
                        for (let j = bracketIdx + 1; j < line.length; j++) {
                            if (!processChar(line[j], lineNumber))
                                return lineMap;
                        }
                    }
                    else {
...
//# sourceMappingURL=line-tracker.js.map